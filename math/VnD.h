...
template <class vType, int size> class VnD 
{
 ...
 public:
  ...
  VnD() {
    for(int i=0;i<size;i++) { v[i] = (vType)0; } }

  VnD( const VnD<vType,size> &vec) {
    for(int i=0;i<size;i++) { v[i] = vec[i];} }
  ...
  vType            x(int nr) const { return v[nr]; }
  vType&           x(int nr)       { return v[nr]; }
...
template <class vType, int size> VnD<vType,size> VnD<vType,size>::getNormalized() const
{
  ...
  vType n = getNorm();
  if (n<=epsilon) { fprintf(stderr,"VnD::getNormalized() ... cannot be normalized!\n"); }
  else {
      for(int i=0;i<size;i++) {
	    q[i] = v[i]/n;
      }
