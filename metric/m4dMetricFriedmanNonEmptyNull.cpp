 ...
 */
bool 
MetricFriedmanNonEmptyNull :: calculateMetric  ( const double* pos )
{
  double chi   = pos[0];
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
bool 
MetricFriedmanNonEmptyNull :: calculateChristoffels  ( const double* pos )
{
  double chi   = pos[0];
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
bool 
MetricFriedmanNonEmptyNull :: calculateChrisD ( const double* pos )
{
  double chi   = pos[0];
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
void
MetricFriedmanNonEmptyNull :: localToCoord ( const double* pos, const double* ldir, double* dir, 
 ...
 */
void 
MetricFriedmanNonEmptyNull :: coordToLocal ( const double* pos, const double* cdir, double* ldir, 
                                             enum_nat_tetrad_type   )
{
  double chi   = pos[0];
  double r     = pos[1];
 ...
 */
bool 
MetricFriedmanNonEmptyNull :: breakCondition ( const double* pos)
{
  bool br = false;
  ...
  double k = (double)mK;
  if (mK==-1)
    if (fabs(1.0+0.25*k*r*r)<1e-12) br = true;
  return br;
}
 ...
 */
double 
MetricFriedmanNonEmptyNull :: testConstraint ( const double y[], const double kappa )
{
  double chi   = y[0];
  double r     = y[1];
  double theta = y[2];
 ...
 */
bool 
MetricFriedmanNonEmptyNull :: setParam ( std::string pName, double val )
{
 ...
 */
bool    
MetricFriedmanNonEmptyNull :: transToTwoPlusOne  ( vec4 p, vec4 &cp )
{
  vec4 tp;
 ...
 */
bool    
MetricFriedmanNonEmptyNull :: report ( const vec4 , const vec4 , std::string &text )
{
  std::stringstream ss;
 ...
 */
void 
MetricFriedmanNonEmptyNull :: setStandardValues( )
{
  mInitPos[0] = 1.0; 
  mInitPos[1] = 10.0;
  mInitPos[2] = M_PI_2;
 ...
 */
double 
MetricFriedmanNonEmptyNull :: calc_R ( const double chi )
{
  if (mK==0)
  {
 ...
 */
double 
MetricFriedmanNonEmptyNull :: calc_dR ( const double chi )
{
  if (mK==0)
  {
	  return 4.5*mC*chi;
 ...
 */
double 
MetricFriedmanNonEmptyNull :: calc_ddR ( const double chi )
{
  if (mK==0)
  {
	  return 4.5*mC;
