 ...
 */
bool 
MetricChazyCurzonRot :: calculateMetric  ( const double* pos )
{
    double rho = pos[1];
    double em2U,k,A;
 ...
 */
bool 
MetricChazyCurzonRot :: calculateChristoffels  ( const double* pos )
{
    double rho = pos[1];
 ...
 */
bool 
MetricChazyCurzonRot :: calculateChrisD ( const double*  )
{
 ...
 */
void
MetricChazyCurzonRot :: localToCoord ( const double* pos, const double* ldir, double* dir,
 ...
 */
void 
MetricChazyCurzonRot :: coordToLocal ( const double* , const double* , double* ,
 ...
 */
bool 
MetricChazyCurzonRot :: breakCondition ( const double* )
{
    return false;
}
 ...
 */
double 
MetricChazyCurzonRot :: testConstraint ( const double y[], const double kappa )
{
    double rho    = y[1];
    ...
    sum += em2U * (exp(2.0*k)*(drho*drho+dz*dz) + rho*rho*dphi*dphi) - 1.0/em2U * (dt + A*dphi)*(dt + A*dphi);
 ...
 */
bool 
MetricChazyCurzonRot :: setParam ( std::string pName, double val )
{
 ...
 */
bool    
MetricChazyCurzonRot :: report ( const vec4 , const vec4 , std::string &text )
{
    std::stringstream ss;
 ...
 */
void 
MetricChazyCurzonRot :: setStandardValues( )
{
    mInitPos[0] = 0.0;
    mInitPos[1] = 6.0;
    mInitPos[2] = 0.0;
 ...
 */
void
MetricChazyCurzonRot :: calcUkA ( const double* pos, double &em2U, double &k, double &A )
{
    double rho = pos[1];
    double z   = pos[3];
 ...
 */
void
MetricChazyCurzonRot :: calcDUka  ( const double* pos, double &dUdrho, double &dUdz,
