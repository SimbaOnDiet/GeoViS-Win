 ...
 */
bool 
MetricKottler :: calculateMetric  ( const double* pos )
{
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
bool 
MetricKottler :: calculateChristoffels  ( const double* pos )
{
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
bool 
MetricKottler :: calculateChrisD ( const double* pos )
{
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
void
MetricKottler :: localToCoord ( const double* pos, const double* ldir, double* dir, 
 ...
 */
void 
MetricKottler :: coordToLocal ( const double* pos, const double* cdir, double* ldir, 
                                enum_nat_tetrad_type   )
{
  double r     = pos[1];
  double theta = pos[2];
 ...
 */
bool 
MetricKottler :: breakCondition ( const double* pos)
{
  bool br = false;

  double r = pos[1];
  if ((r<0.0) || (fabs(1.0-rs/r-mLambda/3.0*r*r) <= eps)) { br=true; }
  return br;
}
 ...
 */
double 
MetricKottler :: testConstraint ( const double y[], const double kappa )
{
  double r = y[1];
  double theta = y[2];
 ...
 */
bool 
MetricKottler :: setParam ( std::string pName, double val )
{
 ...
 */
bool
MetricKottler :: transToEmbedding ( vec4 p, vec4 &ep )
{
  vec4 cp;
 ...
 */
bool
MetricKottler :: setEmbeddingParam    ( std::string name, double val )
{
 ...
 */
bool
MetricKottler :: testEmbeddingParams  ( )
{
  bool allOk = true;
 ...
 */
int
MetricKottler :: getEmbeddingVertices ( std::vector<vec3> &verts,
 ...
 */
void   
MetricKottler :: usePhysicalUnits ( const enum_physical_constants  units )
{
 ...
 */
void   
MetricKottler :: setUnits ( const double speed_of_light, const double grav_const, const double diel_perm )
{
 ...
 */
bool
MetricKottler :: effPotentialValue  ( const vec4 pos, const vec4 cdir ,enum_geodesic_type type, const double x, double &val )
{
  double kappa = 0.0;
 ...
 */
bool
MetricKottler :: totEnergy ( const vec4 pos, const vec4 cdir, const double , double &val )
{
   if (mLambda<0.0)
   {
     if (pos[1]<r1+eps)
 ...
 */
bool    
MetricKottler :: report ( const vec4 pos, const vec4 cdir, std::string &text )
{
  std::stringstream ss;
 ...
 */
void 
MetricKottler :: setStandardValues( )
{
  mInitPos[0] = 0.0; 
  mInitPos[1] = 3.0*rs;
  mInitPos[2] = M_PI_2;
 ...
 */
void
MetricKottler :: calcCriticalPoints ( )
{
  if (mLambda<0.0)
  {
...
bool
MetricKottler :: calcEmbeddingZ ( const double r, double &z )
{
