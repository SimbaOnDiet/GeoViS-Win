 ...
 */
bool 
MetricKerrBL :: calculateMetric  ( const double* pos )
{
    double r     = pos[1];
    double theta = pos[2];
    double m = mass;
 ...
 */
bool 
MetricKerrBL :: calculateChristoffels  ( const double* pos )
{
    double r     = pos[1];
    double theta = pos[2];
    double m = mass;
 ...
 */
bool 
MetricKerrBL :: calculateChrisD ( const double* pos )
{
    double r     = pos[1];
    double theta = pos[2];
    double m = mass;
 ...
 */
void
MetricKerrBL :: localToCoord ( const double* pos, const double* ldir, double* dir, 
...
{ 
    ...
    {
        double w = sqrt(1.0-2.0*mass*r/sigma);
        dir[0] = ldir[0]/w - ldir[3]*pm*2.0*mass*angmom*r*sin(theta)/(w*sqrt(delta)*sigma);
    ...
    {
        double omega = 2.0*mass*angmom*r/A;
        ga = sqrt(A/(sigma*delta));
        dir[0] = sqrt(A/(sigma*delta))*ldir[0];
        dir[1] = ldir[1]*sqrt(delta/sigma);
        dir[2] = ldir[2]/sqrt(sigma);
        dir[3] = ldir[0]*omega*ga +ldir[3]*pm*sqrt(sigma/A)/sin(theta);
    }
}
 ...
 */
void 
MetricKerrBL :: coordToLocal   ( const double* pos, const double* cdir, double* ldir, 
                                 enum_nat_tetrad_type  type )
{
    double r     = pos[1];
    double theta = pos[2];
    ...
    {
        double w = sqrt(1.0-2.0*mass*r/sigma);
        ldir[0] = w*cdir[0] + 2.0*mass*angmom*r*sin(theta)*sin(theta)/(w*sigma)*cdir[3];
 ...
 */
bool 
MetricKerrBL :: breakCondition ( const double* pos)
{
    bool br = false;
 ...
 */
bool
MetricKerrBL :: calcDerivs ( const double y[], double dydx[] )
{
    dydx[0] = y[4];
    dydx[1] = y[5];
    dydx[2] = y[6];
 ...
 */
double 
MetricKerrBL :: testConstraint ( const double y[], const double kappa )
{
    double r = y[1];
    double theta = y[2];
 ...
 */
bool 
MetricKerrBL :: setParam ( std::string pName, double val )
{
 ...
 */
bool    
MetricKerrBL :: transToTwoPlusOne  ( vec4 p, vec4 &cp )
{
    vec4 tp;
 ...
 */
bool
MetricKerrBL :: effPotentialValue  ( const vec4 pos, const vec4 cdir , enum_geodesic_type type, const double x, double &val )
{
    double kappa = 0.0;
 ...
 */
bool
MetricKerrBL :: totEnergy ( const vec4 , const vec4 , const double , double &val )
{
    val = 0.0;
    return true;
}
...
double
MetricKerrBL :: getCircularVelocity( const double r, const enum_nat_tetrad_type  )
{
    double rs = 2.0*mass;
    double a = angmom;
    double a2 = a*a;
...
void  
MetricKerrBL :: getCircularVelocities ( const double r, double &b1, double &b2 ) 
{
    double rs = 2.0*mass;
    double a = angmom;
    double a2 = a*a;
 ...
 */
bool    
MetricKerrBL :: report ( const vec4 pos, const vec4 cdir, std::string &text )
{
    std::stringstream ss;
 ...
 */
bool    
MetricKerrBL :: calcEventHorizon ( double &rp )
{
 ...
 */
bool    
MetricKerrBL :: calcErgosphere ( double theta, double &r0 )
{
 ...
 */
bool    
MetricKerrBL :: calcPhotonOrbit ( double &rphDirect, double &rphRetrograd )
{
    if (angmom>mass)
        return false;
 ...
 */
bool    
MetricKerrBL :: calcMarginStabOrbit ( double &rmsDirect, double &rmsRetrograd )
{
    if (angmom>mass)
        return false;
 ...
 */
bool
MetricKerrBL :: calcMSOvelocities ( double &velDirect, double &velRetrograd )
{
    double rD,rR;
 ...
 */
void 
MetricKerrBL :: setStandardValues( )
{
    mInitPos[0] = 0.0;
    mInitPos[1] = 6.0*mass;
    mInitPos[2] = M_PI_2;
 ...
 */
bool 
MetricKerrBL :: calcSigmaAndDelta ( double r, double theta )
{
    sigma = r*r + angmom*angmom*cos(theta)*cos(theta);
 ...
 */
bool 
MetricKerrBL :: calcA ( double r, double theta )
{
    sigma = r*r + angmom*angmom*cos(theta)*cos(theta);
    A = (r*r+angmom*angmom)*sigma + 2.0*mass*angmom*angmom*r*sin(theta)*sin(theta);
    return true;
}
 ...
 */
bool 
MetricKerrBL :: calcGamma ( double r, double theta, double zeta )
{
    double st2 = sin(theta)*sin(theta);
