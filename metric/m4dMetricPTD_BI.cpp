 ...
 */
bool
MetricPTD_BI :: calculateMetric  ( const double* pos )
{
    double r     = pos[1];
    double theta = pos[2];
    double b = Par_b;
 ...
 */
bool
MetricPTD_BI :: calculateChristoffels  ( const double* pos )
{
    double r     = pos[1];
    double theta = pos[2];
    double b = Par_b;
    double t1 = -r+b;     double  t2 = sin(theta);
    double  t3 = t2*t2;
    double  t5 = cos(theta);
    double  t7 = 1/r;
    double  t9 = 1/t2*t5;
 ...
 */
void
MetricPTD_BI :: localToCoord ( const double* pos, const double* ldir, double* dir,
 ...
 */
void
MetricPTD_BI :: coordToLocal   ( const double* pos, const double* cdir, double* ldir,
                                           enum_nat_tetrad_type )
{
    double r     = pos[1];
    double theta = pos[2];
 ...
 */
bool
MetricPTD_BI :: breakCondition ( const double* pos)
{
    bool br = false;

    if ((pos[1]<0.0) || (pos[1]*pos[1]<=(1.0+eps)*Par_b*Par_b)) { br=true; }
    return br;
}
 ...
 */
void
MetricPTD_BI :: calcConstantsOfMotion (const vec4 pos, const vec4 cdir)
{
 ...
 */
bool
MetricPTD_BI :: effPotentialValue  ( const vec4 pos, const vec4 cdir ,enum_geodesic_type , const double x, double &val )
{
  /*
    double kappa = 0.0;
 ...
 */
bool
MetricPTD_BI :: totEnergy ( const vec4 , const vec4 , const double , double &val )
{
  val = 0.0;
  return true;
}
 ...
 */
double
MetricPTD_BI :: calculateVeffRoot (double C02, double K, double r0)
{
  double r=0;
  double b=Par_b;
  ...
  if (C02==0) {  // only 1st-Order Polynom
    ...
    r=z1;
    if ((z2>r0) && (fabs(r0-z2)< fabs(r0-r))) r= z2;
    if ((z3>r0) && (fabs(r0-z3)< fabs(r0-r))) r= z3;
  }
 ...
 */
bool
MetricPTD_BI :: setParam ( std::string pName, double val )
{
  Metric::setParam(pName,val);
  if (pName=="b") Par_b = val;
  return true;
}
 ...
 */
bool
MetricPTD_BI :: report ( const vec4 pos, const vec4 cdir, std::string &text )
{
  std::stringstream ss;
...
//  ss << "  m_0^2 = " << m0 << std::endl;
  ...
  if (maxrad >= 0)
    ...
    else ss << "  maximum distance from origin..............r_max= infinity" << std::endl;
 ...
 */
void
MetricPTD_BI :: setStandardValues( )
{
  mInitPos[0] = 0.0;
