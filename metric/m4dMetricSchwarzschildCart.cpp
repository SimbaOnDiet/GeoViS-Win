 ...
 */
bool 
MetricSchwarzschildCart :: calculateMetric  ( const double* pos )
{  
  double x = pos[1];
  double y = pos[2];
  double z = pos[3];
 ...
 */
bool 
MetricSchwarzschildCart :: calculateChristoffels  ( const double* pos )
{
  double x = pos[1];
  double y = pos[2];
  double z = pos[3];
 ...
 */
void
MetricSchwarzschildCart :: localToCoord ( const double* pos, const double* ldir, double* dir, 
 ...
 */
void 
MetricSchwarzschildCart :: coordToLocal ( const double* pos, const double* cdir, double* ldir, 
                                          enum_nat_tetrad_type  )
{
  double x = pos[1];
  double y = pos[2];
 ...
 */
bool 
MetricSchwarzschildCart :: breakCondition ( const double* pos)
{
  bool br = false;
  double r = sqrt(pos[1]*pos[1]+pos[2]*pos[2]+pos[3]*pos[3]);

  if ((r<0.0) || (r*r<=(1.0+eps)*rs*rs)) { br=true; }
  return br;
}
 ...
 */
double 
MetricSchwarzschildCart :: testConstraint ( const double y[], const double kappa )
{
  double xx = y[1];
  double yy = y[2];
  double zz = y[3];
  ...
  sum += -c*c*f*y[4]*y[4] + (xx*xx/f+yy*yy+zz*zz)*edrq*y[5]*y[5] + (xx*xx+yy*yy/f+zz*zz)*edrq*y[6]*y[6] + (xx*xx+yy*yy+zz*zz/f)*edrq*y[7]*y[7]
 ...
 */
bool 
MetricSchwarzschildCart :: setParam ( std::string pName, double val )
{
  if (Metric::setParam(pName,val))
    rs = 2.0*mGravConstant*val;
 ...
 */
bool
MetricSchwarzschildCart :: report ( const vec4 , const vec4 , std::string &text )
{
  std::stringstream ss;
 ...
 */
void 
MetricSchwarzschildCart :: setStandardValues ( )
{
  mInitPos[0] = 0.0; 
  mInitPos[1] = 3.0*rs;
  mInitPos[2] = 0.0;
 ...
 */
void   
MetricSchwarzschildCart :: calcLTcoeffs ( const double* pos )
{
  calculateMetric(pos);
