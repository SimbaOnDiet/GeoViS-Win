 ...
 */
bool 
MetricSchwarzschildIsotropic :: calculateMetric  ( const double* pos )
{
 ...
 */
bool 
MetricSchwarzschildIsotropic :: calculateChristoffels  ( const double* pos )
{
 ...
 */
bool 
MetricSchwarzschildIsotropic :: calculateChrisD ( const double* pos )
{
  double drdx,drdy,drdz;
  calc_drho(pos,drdx,drdy,drdz);
  ...
  double t59 = 2.0*(t7*t41+4.0*t43*t44-8.0*t47*t48+2.0*t2*t44*t13-t14*t41)*t14*t20*t38;
  double t60 = drdxdz;      // diff(diff(rho(x,y,z),x),z);
  double t62 = drdz;        // diff(rho(x,y,z),z);
  double t65 = t62*rho_s;
  double t76 = 2.0*(t7*t60+4.0*t43*t62-8.0*t47*t65+2.0*t2*t62*t13-t14*t60)*t14*t20*t38;
  double t77 = drdydy;      // diff(diff(rho(x,y,z),y),y);
  double t79 = t44*t44;
  double t82 = t6*t79;
  double t93 = drdydz;      // diff(diff(rho(x,y,z),y),z);
  double t98 = t6*t44;
  double t109 = 2.0*(t7*t93+4.0*t1*t44*t62-8.0*t98*t65+2.0*t44*t62*t13-t14*t93)*t14*t20*t38;
  ...
  chrisD[0][0][0][3] = 0.0;
  chrisD[0][0][1][0] = 0.0;
  chrisD[0][0][1][1] = -2.0*(4.0*t1*t3+t7*t8-8.0*t10*rho_s-t8*t14+2.0*t3*t13)*t14*t20*t38;
  chrisD[0][0][1][2] = -t59;
  chrisD[0][0][1][3] = -t76;
  chrisD[0][0][2][0] = 0.0;
  chrisD[0][0][2][1] = -t59;
  chrisD[0][0][2][2] = -2.0*(t7*t77+4.0*t1*t79-8.0*t82*rho_s+2.0*t79*t13-t14*t77)*t14*t20*t38;
  chrisD[0][0][2][3] = -t109;
  chrisD[0][0][3][0] = 0.0;
  chrisD[0][0][3][1] = -t76;
  chrisD[0][0][3][2] = -t109;
  chrisD[0][0][3][3] = -2.0*(t7*t110+4.0*t1*t112-8.0*t115*rho_s+2.0*t112*t13-t14*t110)*t14*t20*t38;
  chrisD[0][1][0][0] = 0.0;
  chrisD[0][1][0][1] = -t137;
  chrisD[0][1][0][2] = -t145;
  chrisD[0][1][0][3] = -t153;
 ...
 */
void
MetricSchwarzschildIsotropic :: localToCoord ( const double* pos, const double* ldir, double* dir,
 ...
 */
void 
MetricSchwarzschildIsotropic :: coordToLocal ( const double* pos, const double* cdir, double* ldir,
                                               enum_nat_tetrad_type  )
{
  double rho = calc_rho(pos);
 ...
 */
bool 
MetricSchwarzschildIsotropic :: breakCondition ( const double* pos )
{
  bool br = false;
 ...
 */
double 
MetricSchwarzschildIsotropic :: testConstraint ( const double y[], const double kappa )
{
 ...
 */
bool   
MetricSchwarzschildIsotropic :: calcProduct ( const double* pos, const double* u, const double* v, double &prod, bool )
{
  prod = 0.0;
 ...
 */
bool 
MetricSchwarzschildIsotropic :: setParam ( std::string pName, double val )
{
 ...
 */
bool    
MetricSchwarzschildIsotropic :: report ( const vec4 pos, const vec4 , std::string &text )
{
  std::stringstream ss;
...
/*! Calculate beta for circular orbit.
 * \param pos : current position.
 * \param beta : reference to velocity.
 * \return true : if rho>=rho_lso.
 */
bool
MetricSchwarzschildIsotropic :: calcBetaOfCircOrbit ( const double* pos, double &beta )
{
   calc_orbits();
 ...
 */
void 
MetricSchwarzschildIsotropic :: setStandardValues( )
{
  mInitPos[0] = 0.0; 
  mInitPos[1] = 6.0;
  mInitPos[2] = 0.0;
 ...
 */
double 
MetricSchwarzschildIsotropic :: calc_rho ( const double* pos )
{
  rho = sqrt(pos[1]*pos[1] + pos[2]*pos[2] + pos[3]*pos[3]);
  return rho;
}

/*! Calculate first derivatives of rho.
  \param pos : pointer to position.
  \param drdx : diff(rho,x).
  \param drdy : diff(rho,y).
  \param drdz : diff(rho,z).
 */
void
MetricSchwarzschildIsotropic :: calc_drho  ( const double* pos, double &drdx, double &drdy, double &drdz )
{
   rho = calc_rho(pos);
   drdx = pos[1]/rho;
   drdy = pos[2]/rho;
...
/*! Calculate second derivatives of rho.
  \param pos : pointer to position.
  \param drdxdx : diff(rho,x,x).
  \param drdxdy : diff(rho,x,y).
  \param drdxdz : diff(rho,x,z).
  \param drdydy : diff(rho,y,y).
  \param drdydz : diff(rho,y,z).
  \param drdzdz : diff(rho,z,z).
 */
void
MetricSchwarzschildIsotropic :: calc_d2rho ( const double* pos, double &drdxdx, double &drdxdy, double &drdxdz,
...
void
MetricSchwarzschildIsotropic :: calc_orbits ( )
{
