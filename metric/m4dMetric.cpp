 ...
 */
void 
Metric :: getCoordNames ( std::string names[4] )
{
    for(int i=0; i<4; i++)
 ...
 */
bool  
Metric :: calculateMetric ( const vec4 pos )
{
 ...
 */
bool  
Metric :: calculateChristoffels  ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool 
Metric :: calculateChrisD ( const double* )
{
 ...
 */
bool   
Metric :: calculateChrisD ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool
Metric :: calculateRiemann ( const double* )
{
    return false;
}
 ...
 */
bool
Metric :: calculateRiemann  ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool  
Metric :: calculateWeyl ( const double* )
{
    return false;
}
 ...
 */
bool 
Metric :: calculateWeyl ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool 
Metric :: calculateRicci ( const double*  )
{
    return false;
}
 ...
 */
bool 
Metric :: calculateRicci ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool 
Metric :: calculateRicRotCoeffs ( const double*  )
{
    return false;
}
 ...
 */
bool 
Metric :: calculateRicRotCoeffs ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
bool 
Metric :: calculateContrRRC ( const double*  )
{
    return false;
}
 ...
 */
bool 
Metric :: calculateContrRRC ( const vec4 pos )
{
    double p[4] = { pos[0],pos[1],pos[2],pos[3] };
 ...
 */
double  
Metric :: getMetricCoeff ( const int mu, const int nu )
{
 ...
 */
double  
Metric :: getChristoffel ( const int mu, const int nu, const int kappa )
{
 ...
 */
double 
Metric :: getChrisD ( const int mu, const int nu, const int kappa, const int tau )
{
 ...
 */
double
Metric :: getRiemCoeff ( const int mu, const int nu, const int rho, const int sigma )
{
 ...
 */
double 
Metric :: getWeylCoeff ( const int mu, const int nu, const int rho, const int sigma )
{
 ...
 */
double 
Metric :: getRicciCoeff ( const int mu, const int nu )
{
    return ric[mu][nu];
}
 ...
 */
double 
Metric :: getRicRotCoeff ( const int i, const int j, const int k )
{
    return rrc[i][j][k];
}
 ...
 */
double 
Metric :: getContrRRCCoeff ( const int j )
{
    return crrc[j];
}
 ...
 */
void 
Metric :: getNatTetrad   ( const vec4 pos, vec4 &e0, vec4 &e1, vec4 &e2, vec4 &e3, 
...
void    
Metric :: getNatDualTetrad ( const vec4 pos, vec4 &t0, vec4 &t1, vec4 &t2, vec4 &t3,
                             enum_nat_tetrad_type  type )
{
 ...
 */
void 
Metric :: localToCoord ( const vec4 pos, const vec4 ldir, vec4 &cdir, 
                         enum_nat_tetrad_type type )
{
 ...
 */
void 
Metric :: coordToLocal ( const vec4 pos, const vec4 cdir, vec4 &ldir, 
                         enum_nat_tetrad_type  type  )
{
 ...
 */
bool
Metric :: breakCondition( const vec4 pos )
{
 ...
 */
bool
Metric :: calcDerivs ( const double* y, double* dydx )
{
 ...
 */
bool 
Metric :: calcDerivsPar  ( const double* y, double* dydx )
{
    register int mu,j,k,l;
 ...
 */
bool  
Metric :: calcDerivsSachsJacobi ( const double* , double* )
{
    return false;
}
 ...
 */
bool 
Metric :: calcDerivsFW   ( const double* , const double* , double*  )
{
    return false;
}
 ...
 */
double 
Metric :: testConstraint ( const double y[], const double kappa )
{
    calculateMetric(y);
 ...
 */
bool
Metric :: resize ( double*, double , double )
{
    return false;
}
 ...
 */
void 
Metric :: usePhysicalUnits  ( const enum_physical_constants  units )
{
 ...
 */
void 
Metric :: setUnits ( const double speed_of_light, const double grav_const, const double diel_perm )
{
 ...
 */
bool    
Metric :: localNullDir  ( const double pm, const double* l3dir, double* l4dir )
{
 ...
 */
bool    
Metric :: localNullDir  ( const double pm, const vec3 l3dir, vec4 &l4dir )
{
 ...
 */
bool    
Metric :: localFourVel  ( const double pm, const double* l3vel, double* l4vel )
{
 ...
 */
bool    
Metric :: localFourVel  ( const double pm, const vec3 l3dir, vec4 &l4dir )
{
    double a3dir[3] = {l3dir[0],l3dir[1],l3dir[2]};
    double a4dir[4];
 ...
 */
bool    
Metric :: calcProduct ( const double* pos, const double* u, const double* v, double &prod, bool preCalcMetric )
{
    prod = 0.0;
 ...
 */
bool    
Metric :: calcProduct ( const vec4 pos, const vec4 u, const vec4 v, double &prod, bool preCalcMetric )
{
    prod = 0.0;
    if (breakCondition(pos))
        return false;
...
void Metric :: setCoordType ( enum_coordinate_type coord_type )
{
    ...
    switch (mCoordType)
    {
        ...
        case enum_coordinate_spherical: {
            ...
            mScoordType[2].min=0;
            mScoordType[2].max=1.0*M_PI; //ToDo, Checken, ob das richtig ist
 ...
 */
double  
Metric :: coordDiff ( const unsigned int coordNum, const double p1, const double p2 )
{
    assert( coordNum<4 );
    double diff;
    ...
    if (mScoordType[coordNum].type == enum_scoord_periodic)
    {
        ...
        if (diff>0.5*mScoordType[coordNum].max) diff = diff-mScoordType[coordNum].max;
        else if (diff<-0.5*mScoordType[coordNum].max) diff = diff+mScoordType[coordNum].max;
    }
    else
    {
        diff = p2-p1;
 ...
 */
bool    
Metric :: calcSepDist ( const vec4 p1, const vec4 p2, double &spaceDist, double &timeDist )
{
    ...
    for (unsigned int i=0; i<4; i++)
    {
        ...
        {
            if ( (mScoordType[i].character==enum_cchar_spacelike) && (mScoordType[j].character==enum_cchar_spacelike) )
                spaceDist += g_compts[i][j]*coordDiff(i,p1[i],p2[i])*coordDiff(j,p1[j],p2[j]);

            if ( (mScoordType[i].character==enum_cchar_timelike) && (mScoordType[j].character==enum_cchar_timelike) )
                timeDist  -= g_compts[i][j]*coordDiff(i,p1[i],p2[i])*coordDiff(j,p1[j],p2[j]);
        }
    }
    
    if (spaceDist>=0.0)
        spaceDist = sqrt(spaceDist);

    if (timeDist >=0.0)
        timeDist  = sqrt(timeDist );
    return true;
}

bool
Metric :: calcTidalMatrix ( const double* pos, const double* e0, const double* e1, const double* e2, const double* e3 )
{
    return calcTidalMatrix(vec4(pos),vec4(e0),vec4(e1),vec4(e2),vec4(e3));
}

bool
Metric :: calcTidalMatrix ( const vec4 pos, const vec4 e0, const vec4 e1, const vec4 e2, const vec4 e3 )
{
    tidalMatrix.setNull();
                ...
                for(int nu=0; nu<4; nu++)
                    for(int rho=0; rho<4; rho++)
                        for(int sigma=0; sigma<4; sigma++)
                            sum += tt.getCoeff(mu,i)*riem[mu][nu][rho][sigma]*e0[nu]*e0[rho]*ee.getCoeff(j,sigma);
            tidalMatrix.setCoeff(i,j,sum*mSpeedOfLight*mSpeedOfLight);
        }
    }
    return true;
}
...
double
Metric :: getTidalMatrixCoeff ( const int i, const int j )
{
    return tidalMatrix.getCoeff(i,j);
}

void
Metric :: getTidalMatrix ( mat4 &m )
{
    m = mat4(tidalMatrix);
}
 ...
 */
bool    
Metric :: addParam ( std::string pName, double val )
{
    lowCase(pName);
 ...
 */
bool
Metric :: setParam ( std::string pName, double val )
{
    lowCase(pName);
    mParamItr = mParam.find(pName);
    if (mParamItr == mParam.end())
 ...
 */
bool
Metric :: getParam ( std::string pName, double &val )
{
    lowCase(pName);
    mParamItr = mParam.find(pName);
    if (mParamItr == mParam.end())
 ...
 */
bool 
Metric :: getParam( int pNr, std::string& pName, double& val )
{
...
bool
Metric :: setParam  ( int pNr, double val )
{
    if ( pNr >= 0 && pNr < mNumParam )
    {
        std::map<std::string,double>::iterator p_itr = mParam.begin();
 ...
 */
void 
Metric :: getParamNames  ( std::vector<std::string> &names )
{
    if (!names.empty())
        names.clear();
...
int
Metric :: getParamNum ( std::string name )
{
    int count = 0;
    mParamItr = mParam.begin();
    while (mParamItr != mParam.end())
 ...
 */
int
Metric :: getLocTedTypes ( std::vector<enum_nat_tetrad_type> &locted )
{
    locted = mLocTeds;
 ...
 */
int
Metric :: getDrawTypes ( std::vector<enum_draw_type> &drawTypes )
{
 ...
 */
enum_draw_type
Metric :: getCurrDrawType ( int num )
{
 ...
 */
enum_draw_type        
Metric :: getCurrDrawType ( std::string name )
{
    int num = 0;
 ...
 */
bool  
Metric :: transToEmbedding ( vec4 , vec4& )
{
    return false;
}
 ...
 */
bool    
Metric :: transToTwoPlusOne  ( vec4 p, vec4 &cp )
{
    vec4 tp;
 ...
 */
bool    
Metric :: transToCustom ( vec4 p, vec4 &cp )
{
    cp = p;
    return true;
}
 ...
 */
bool
Metric :: addEmbeddingParam    ( std::string name, double val )
{
    lowCase(name);
 ...
 */
bool
Metric :: setEmbeddingParam    ( std::string name, double val )
{
    lowCase(name);
    mEmbParamItr = mEmbParam.find(name);
    if (mEmbParamItr == mEmbParam.end())
 ...
 */
bool
Metric :: getEmbeddingParam    ( std::string name, double &val )
{
    lowCase(name);
    mEmbParamItr = mEmbParam.find(name);
    if (mEmbParamItr == mEmbParam.end())
 ...
 */
void    
Metric :: getEmbeddingNames    ( std::vector<std::string> &names )
{
    if (!names.empty())
        names.clear();
 ...
 */
bool
Metric :: getAllEmbeddingParams  ( std::vector<std::string> &names, std::vector<double> &params )
{
    if (!params.empty())
        params.clear();
 ...
 */
bool
Metric :: getEmbeddingMap ( std::map<std::string,double> &params )
{
    if (mEmbParam.size()==0)
        return false;
 ...
 */
int
Metric :: getEmbeddingVertices ( std::vector<vec3>&,
 ...
 */
bool
Metric :: effPotentialValue  ( const vec4 , const vec4 , enum_geodesic_type ,const double , double & )
{
    return false;
}

bool
Metric :: totEnergy ( const vec4 , const vec4 , const double , double & )
{
    return false;
}
 ...
 */
double
Metric :: getCircularVelocity ( const double , const enum_nat_tetrad_type  )
{
 ...
 */
bool
Metric :: report ( const vec4 , const vec4 , std::string &text )
{
    text = std::string();
    return false;
}
 ...
 */
void
Metric :: contrChrisVecVec  ( const double* , const double* v, const double* w, double* z, bool )
{   
    for(int mu=0; mu<4; mu++)
    {
        z[mu] = 0.0;
 ...
 */
void  
Metric :: contrChrDVecVecVec ( const double* , const double* u, const double* v, const double* w, double* z, bool )
{
    for(int mu=0; mu<4; mu++)
    {
        z[mu] = 0.0;
