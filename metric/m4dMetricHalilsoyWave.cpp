 ...
 */
bool
MetricHalilsoyWave :: calculateMetric ( const double* pos )
{
  double rho = pos[1];
  calcMetricFunc(pos);
 ...
 */
bool
MetricHalilsoyWave :: calculateChristoffels ( const double* pos )
{
  double rho = pos[1];
  calcMetricFunc(pos);
 ...
 */
bool
MetricHalilsoyWave :: calculateChrisD ( const double*  )
{
	return false;
}
 ...
 */
void
MetricHalilsoyWave :: localToCoord ( const double* pos, const double* ldir, double* dir,
 ...
 */
void
MetricHalilsoyWave :: coordToLocal ( const double* pos, const double* cdir, double* ldir,
                                      enum_nat_tetrad_type )
{
  double rho = pos[1];
  calcMetricFunc(pos);
 ...
 */
bool
MetricHalilsoyWave :: breakCondition ( const double*  )
{
  bool br = false;
  return br;
}
 ...
 */
bool
MetricHalilsoyWave :: setParam ( std::string pName, double val )
{
 ...
 */
double
MetricHalilsoyWave :: testConstraint ( const double y[], const double kappa )
{
  double rho  = y[1];
  double dt   = y[4];
  double drho = y[5];
  ...
  calcMetricFunc(y);
  sum += fV*(exp(2.0*fK)*(drho*drho - dt*dt) + rho*rho*dphi*dphi) + (dz+fA*dphi)*(dz+fA*dphi)/fV;
  return sum;
}
 ...
 */
bool
MetricHalilsoyWave :: report ( const vec4, const vec4, std::string &text )
{
  std::stringstream ss;
 ...
 */
void
MetricHalilsoyWave :: setStandardValues ()
{
  mInitPos[0] = 0.0;
  mInitPos[1] = 5.0;
  mInitPos[2] = 0.0;
 ...
 */
void
MetricHalilsoyWave :: calcMetricFunc ( const double* pos )
{
  double t   = pos[0];
  double rho = pos[1];
 ...
 */
void
MetricHalilsoyWave :: calcDiffMetricFunc ( const double* pos )
{
  double t   = pos[0];
  double rho = pos[1];
  ...
  fV_tt = 2.0*cha*cha*mC*b0*ct*emb0 + 4.0*cha*cha*mC*mC*b0*b0*st*st*emb0 - 2.0*sha*sha*mC*b0*ct*epb0 + 4.0*sha*sha*mC*mC*b0*b0*st*st*epb0;
  
  fV_rho = 2.0*cha*cha*mC*b1*ct*emb0 - 2.0*sha*sha*mC*b1*ct*epb0;
  fV_rhorho = 2.0*cha*cha*mC*(b0-b1/rho)*ct*emb0 + 4.0*cha*cha*mC*mC*b1*b1*ct*ct*emb0 - 2.0*sha*sha*mC*(b0-b1/rho)*ct*epb0 + 4.0*sha*sha*mC*mC*b1*b1*ct*ct*epb0;
  
  fV_trho = -2.0*cha*cha*mC*b1*st*emb0 + 4.0*cha*cha*mC*mC*b0*st*b1*ct*emb0 + 2.0*sha*sha*mC*b1*st*epb0 + 4.0*sha*sha*mC*mC*b0*st*b1*ct*epb0;
  ...
  fK_rhorho = -mC*mC*(-b0*b0 + 2.0*b0*b1*rho + b1*b1*ct*ct + b0*b0*ct*ct - 4.0*rho*b0*b1*ct*ct);
