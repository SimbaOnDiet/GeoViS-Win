...
{    
    ...
    while ((int)points.size()<maxNumPoints && breakType == enum_break_none)
    {
        ...
        h = hnext;
        if (h > mhmax)  h= mhmax;//Maximale Schrittweite, um "runde" Geodäten zu bekommen
    }
                                      ...
                                      std::vector<vec4> &points, std::vector<vec4> &dirs, std::vector<double> &epsilons, std::vector<double> &lambda )
{
    ...
    if (!points.empty())      points.clear();
    if (!dirs.empty())        dirs.clear();
    if (!epsilons.empty())    epsilons.clear();
    if (!lambda.empty())      lambda.clear();
    ...
    while ((int)points.size()<maxNumPoints && breakType == enum_break_none)
    {
        ...
        h = hnext;
        if (h > mhmax)  h= mhmax;//Maximale Schrittweite, um "runde" Geodten zu bekommen
    }
    if ((int)points.size()>=maxNumPoints)
        breakType = enum_break_num_exceed;
                                 ...
                                 std::vector<vec5> &jacobi, vec5 &maxJacobi )
{
    ...
    if (!dirs.empty())        dirs.clear();
    if (!lambda.empty())      lambda.clear();
    if (!sachs0.empty())      sachs0.clear();
    if (!sachs1.empty())      sachs1.clear();
