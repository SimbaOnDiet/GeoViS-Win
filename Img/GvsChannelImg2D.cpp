...
void GvsChannelImg2D :: resize(int newWidth, int newHeight , bool resizeData) {
    ...
    if ( (imgWidth != newWidth) || (imgHeight != newHeight) )  {
        ...
        for ( double y = 0.0; y < newHeightD; y += 1.0 ) {	//Interpolation was made here to enlarge or minify the image
            ...
            for ( double x = 0.0; x < newWidthD; x += 1.0 ) {
                ...
                interpolColor = sampleColor( int(oldX+0.5), int(oldY+0.5) );//FG (richtiges Runden) ENG:Straight Rounding
...
double GvsChannelImg2D :: sampleValue ( int x, int y ) const {	//I will look it back later LABEL
    if (x < 0) {
...
{
    // col.Print();
    int indx = y * imgWidth + x;    //PROBLEMATIC
...
void GvsChannelImg2D :: setChannel  ( int x, int y, int chNum, uchar ch ) {
    int indx = y * imgWidth + x;
    imgData[indx+imgChanSize*chNum] = ch;   //? PROBLEMATIC
}
...
void GvsChannelImg2D::writeIntersecData( const char* filename , GvsCamFilter filter ) {
    ...
    } else  if (filter==gvsCamFilterRGBjac) {
        nc = NUM_JAC_CHANNELS;
