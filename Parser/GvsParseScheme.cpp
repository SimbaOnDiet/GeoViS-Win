...
bool GvsParseScheme :: getParameter ( const char* name, double* dval ) {
    ...
    if (testArgType(name,num))
    {
        ...
        if ((getAllowedParamType(name)!=paramType) && (paramType!=gp_string_gvs))
        {
            std::cerr << name << " " << paramType << std::endl;
            msg = name;
            msg.append(": double erwarted! ");
            ...
            if (is_vector(arg_value)) {
                ...
                if (is_vector(elem)) {
                    ...
                    if ((n==2) && (m==3)) {
                        ...
                        scheme_error("matrix format cannot yet be read.");
                    }
            ...
            // -------------------------------------
                ...
                if (is_number(pval)) {
                    ...
                    if (is_vector(elem))
                    {
                        ...
                        {
                            scheme_error("Matrix-Format kann nicht gelesen werden");
                        }
                    }
