 ...
 * @return
 */
int GvsParser::getNumDevices ( void ) const {
 ...
 */
void
GvsParser :: getDevice ( GvsDevice *device, unsigned int k )
{
    assert ( k < gpDevice.size() );

    device->metric = gpDevice[k]->metric;
    // device->spacetime = gpDevice[k]->spacetime;
...
void GvsParser::read_scene(const char* name) {
	...
	std::string file_name =  "Parser\\init.scm";
    if (!scheme_init(&sc)) {
    ...
    scheme_define(&sc, sc.global_env, mk_symbol(&sc, "set-parameter"),
                  mk_foreign_func(&sc, gvsP_set_parameter));

    scheme_define(&sc, sc.global_env, mk_symbol(&sc, "get-parameter"),
                  mk_foreign_func(&sc, gvsP_get_parameter));
...
std::string GvsParser::getFullPathname() {
    ...
    //char proc[512];
	wchar_t proc[512];
    int ch = GetModuleFileName(NULL,proc,512);  //int ch = readlink(_link.c_str(),proc,512);
    if (ch != 1) {
        proc[ch] = 0;
        //path = proc;
		USES_CONVERSION;
