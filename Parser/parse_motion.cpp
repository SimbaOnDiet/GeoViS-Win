...
void gvsP_init_geodMotion (GvsParseScheme* gP) {
    ...
    gP->setAllowedName("lftype",gp_string_string,0);
    ...
    UNUSED_ATTR bool haveE = false;     // Tetrade ist in Koordinaten gegeben
    bool haveEloc = false;  // Tetrade ist bzgl lokaler Tetrade gegeben
    ...
    if (!haveTetrad)
    {
        ...
        haveLFType = gP->getParameter("lftype",lfTypeName);
        if (haveLFType) {
            int num = searchCharArray((char**)&m4d::stl_nat_tetrad_types,m4d::NUM_ENUM_NAT_TETRAD_TYPES,lfTypeName.c_str());
            if (num == -1)
    ...
    GvsTypeID tid = {gtMotion,gpMotion.size()-1,gpMotion[gpMotion.size()-1]};
