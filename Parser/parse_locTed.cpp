...
pointer gvsP_local_tetrad (scheme *sc, pointer args) {
    ...
    if (gvsParser->getParameter("localframe",locFrame)) {
        for (int lf=0; lf<m4d::NUM_ENUM_NAT_TETRAD_TYPES; lf++) {
            if (std::string(m4d::stl_nat_tetrad_types[lf])==locFrame) locTed->setInCoords(false,(m4d::enum_nat_tetrad_type)lf);
        }
    }

    // Read velocity
    if (haveLocVel) {
        //     std::cerr << "parse-locTed: locVel: " << locVel[0] << " " << locVel[1] << " " << locVel[2] << std::endl;
        locTed->setLocalVel(m4d::vec3(locVel[0],locVel[1],locVel[2]));
    } else {
        locTed->setVelocity(m4d::vec4(vel[0],vel[1],vel[2],vel[3]));
    }
    ...
    GvsTypeID tid = {gtLocTed,gpLocalTetrad.size()-1,gpLocalTetrad[gpLocalTetrad.size()-1]};
