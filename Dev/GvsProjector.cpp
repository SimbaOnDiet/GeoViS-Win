...
//    LOG.printf(5,"Pixel: %4d %4d ************************\n",static_cast<int>(x),static_cast<int>(y));
    ...
    GvsColor col = GvsColor(0.0,0.0,0.2);
...
bool GvsProjector :: SetParam( const std::string pName, m4d::vec4 pt ) {
    bool isOkay = GvsBase::SetParam(pName,pt);
    if (isOkay && pName=="position") {
        setPosition(pt);
    }
    return isOkay;
}

bool GvsProjector::SetParam( const std::string pName, int nr ) {
    bool isOkay = GvsBase::SetParam(pName,nr);
    if (isOkay && pName=="actualpos") {
        setActualPos(nr);
    }
    return isOkay;
}
