...
void GvsSampleMgr::setRegionToImage() {
    ...
    sampleRegionUR.setX(1,sampleDevice->camera->GetResolution().x(1)-1);    //Everything included?
...
void GvsSampleMgr :: setRegion ( const m4d::ivec2 &corner1, const m4d::ivec2 &corner2 ) {
    ...
    // Is  LL < UR ? If this is the case, swap the conflicting components so as to clear conflicts.
...
bool GvsSampleMgr::putNextPixel() {
    if ( ++samplePixCoord.x(0) > sampleRegionUR.x(0) ) {
        if ( ++samplePixCoord.x(1) > sampleRegionUR.x(1) ) return false;	//PROBLEMATIC?
